...
Organizations rely on multiple threat intelligence sources to stay informed about emerging vulnerabilities and attack techniques. Monitoring these feeds manually is time-consuming, often results in duplicate reporting, and rarely provides consistent threat classification.

This project automates the collection, normalization, deduplication, classification, and delivery of cybersecurity intelligence through a lightweight serverless architecture. It aggregates multiple sources, enriches alerts with MITRE mappings, and delivers curated notifications through Telegram.

By combining GitHub Actions with Amazon EventBridge, the platform provides a cost-effective, low-maintenance automation workflow while preserving architectural simplicity and operational reliability.

## Why This Project?

This repository was created as a practical demonstration of security architecture principles including event-driven automation, serverless design, resilient integrations, lightweight state management, and standards-based threat classification. The emphasis is on architectural decisions and engineering trade-offs rather than implementation complexity.
...
* **`verify_dedup.py`**: Unit test suite validating title-and-summary deduplication logic and threshold boundaries.
* **`verify_mitre.py`**: Unit test suite validating framework switching, title exclusions, and tactic/technique formatting.
...
```
To run the automated test suite verifying deduplication and MITRE rules:
```bash
